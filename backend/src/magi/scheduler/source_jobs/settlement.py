"""Source sync job settlement for the scheduler repository."""
...
def _continuation_identifiers(parent_job_id: str) -> tuple[str, str]:
    continuation_id = uuid.uuid5(
        uuid.NAMESPACE_URL,
        f"magi:source-sync-continuation:{parent_job_id}",
    ).hex
    return (
        f"source_sync_job_{continuation_id}",
...
class _SourceSyncJobSettlementMixin:
    async def settle_source_sync_job_failure(
        self,
        job_id: str,
        *,
        error: str,
        ...
        async with host._connect() as db:
            ...
            if execution is None:
                await db.rollback()
                raise RuntimeError(
                    f"Running source sync execution not found: {job['execution_id']}"
                )
            ...
            if target_cursor.rowcount != 1:
                await db.rollback()
                raise RuntimeError(
                    "Source sync target state not found: "
                    f"{job['target_type']}:{job['target_key']}"
                )
            await db.commit()
            return requeued

    async def settle_source_sync_job_success(
        self,
        job_id: str,
        *,
        ...
        async with host._connect() as db:
            ...
            try:
                ...
                job = await job_cursor.fetchone()
                if job is None:
                    raise RuntimeError(f"Source sync job not found: {job_id}")
                if str(job["status"]) == "success":
                    ...
                    if continue_sync != (continuation is not None):
                        raise RuntimeError(
                            "Source sync continuation contract changed after success: "
                            f"{job_id}"
                        )
                    await db.rollback()
                    return SourceSyncSuccessSettlement(
                        committed=False,
                        continuation_job_id=(
                ...
                if (
                    ...
                    raise RuntimeError(
                        f"Source sync success settlement lost state for job: {job_id}"
                    )

                if continue_sync:
                    continuation_schedule_id = (
                        "source-sync-continuation:"
