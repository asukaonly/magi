...
describe("EMPTY_STATE_PRIORITY_PLUGINS", () => {
  ...
  it("exposes display metadata for every priority plugin", () => {
    ...
    for (const pluginId of pluginIds) {
      const meta: EmptyStatePluginMeta | undefined =
        getEmptyStatePluginMeta(pluginId);
  ...
  it("defines complete metadata for every first-context recommendation", () => {
    ...
    for (const pluginId of FIRST_CONTEXT_PRIORITY_PLUGINS) {
      ...
      expect(
        meta?.firstContextValueKey,
        `first-context value for ${pluginId}`,
      ).toBeTypeOf("string");
      expect(
        meta?.recommendationCategory,
        `category for ${pluginId}`,
      ).toBeTypeOf("string");
    }
  });
  ...
  it("keeps standard empty-state copy separate from first-context copy", () => {
    ...
    expect(enOnboarding.emptyState.firstContextHeading).toBe(
      "Recommended starting point",
    );
    expect(zhOnboarding.emptyState.heading).toBe("让 magi 看看你的活动");
    expect(zhOnboarding.emptyState.firstContextHeading).toBe(
      "推荐从这里开始",
    );
