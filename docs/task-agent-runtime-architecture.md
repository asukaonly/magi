...
Direct chat without tools performs a final hard-capacity check immediately before calling the active core model. If the assembled request no longer fits, it first drops raw history while retaining the active session summary and origin, then falls back to the current turn only. If the fixed system prompt and current turn still exceed the model input capacity, the request fails locally without contacting the provider.
