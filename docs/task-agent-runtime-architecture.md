...
`ChatTranscriptSummarizer` runs after chat responses are persisted, off the response critical path. It estimates the prompt-history token footprint, summarizes the older raw range when the retained tail grows beyond the trigger budget, and activates a new `token_budget` summary. If summary A is active, the next summary is generated from summary A plus the newly covered raw range, then summary B supersedes A for normal prompt reads. Summary output scales with the active core model's input capacity instead of using a fixed model-independent limit: it targets 5% of input capacity, with a 1,024-token floor and a 16,384-token ceiling, and never exceeds the summary model's own output limit.
