"""Tests for the authoritative source-to-L1 commit boundary."""
...
def _source_event() -> Event:
    payload = SourceEventEmitted(
        ...
        memory_event_type="TEST_SOURCE_EVENT",
    )
    return Event(
...
async def _commit(
    committer: SourceEventCommitter,
    *,
...
async def test_commit_returns_persisted_receipt_after_l1_confirmation() -> None:
    ...
    receipt = await _commit(SourceEventCommitter(unified_memory=memory))

    assert receipt.event_id == "source-event-1"
    assert receipt.outcome is SourceCommitOutcome.PERSISTED
...
async def test_commit_returns_canonical_duplicate_receipt() -> None:
    ...
    assert receipt.outcome is SourceCommitOutcome.DUPLICATE
...
async def test_governed_skip_is_a_terminal_accepted_outcome(skip_reason: str) -> None:
    ...
    assert receipt.outcome is SourceCommitOutcome.GOVERNED_SKIP
...
async def test_pre_clear_source_event_is_skipped_without_touching_memory() -> None:
    ...
    receipt = await _commit(
        SourceEventCommitter(unified_memory=memory),
        clear_generation=3,
...
async def test_missing_l1_confirmation_requires_retry() -> None:
    ...
    with pytest.raises(SourceCommitDeferredError, match="without L1 confirmation"):
        await _commit(SourceEventCommitter(unified_memory=memory))


@pytest.mark.asyncio
async def test_l1_exception_propagates_to_source_job() -> None:
    memory = AsyncMock()
