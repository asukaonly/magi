  ...
  it('renders only an archive action on each card', async () => {
    vi.mocked(memoryStoriesApi.list).mockResolvedValue({
      items: [
        {
          summary_id: 'ins-1', summary_type: 'insight', summary_category: 'state_change',
          title: '', content: 'insight body', period_start: 0, period_end: 1700100000,
          updated_at: 1700100000, review_state: 'pending_confirmation',
          insight_key: null, insight_metadata: {}, evidence_event_count: 2,
        },
        {
    ...
    renderPage();
    await waitFor(() => screen.getByText('insight body'));
    ...
    // Both cards have an archive button (getAllByLabelText returns >= 2)
    expect(screen.getAllByLabelText(/收起|Archive/i).length).toBeGreaterThanOrEqual(2);
  });
});
