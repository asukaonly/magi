...
async def test_interrupted_global_clear_discards_batch_manifests_before_resume(
    ...
    chat_read_service = SimpleNamespace(
        aget_interrupted_global_clear_count=AsyncMock(return_value=4)
    )
    ...
    monkeypatch.setattr(
        "magi.chat.get_chat_read_service",
...
async def test_normal_startup_resumes_batch_manifests(monkeypatch) -> None:
    chat_read_service = SimpleNamespace(
        aget_interrupted_global_clear_count=AsyncMock(return_value=None)
    )
    ...
    await AgentRuntimeModule._resume_batch_jobs(SimpleNamespace(manager=manager))
