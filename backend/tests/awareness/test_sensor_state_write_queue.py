"""Tests for the sensor state fingerprint write queue."""
...
@pytest.mark.asyncio
async def test_batches_fingerprints_by_sensor(fake_store):
    queue = SensorStateWriteQueue(
...
async def test_requires_start_before_enqueue(fake_store):
    queue = SensorStateWriteQueue(sensor_state_store=fake_store)
