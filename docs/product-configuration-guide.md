...
- a dedicated first-context step that gives equal weight to answering one ordinary-life question or connecting optional user-approved data sources
...
The user-authored path should ask one concrete, ordinary-life question at a time, allow the user to switch questions, and keep the answer optional. Questions must not assume that the user is employed, managing a project, or using Magi for work. The submitted text must remain the user's exact visible chat message; the selected question is carried as hidden turn context so short answers still make sense without fabricating words in the transcript. Submission uses the normal chat persistence and memory pipeline, with one stable session and turn identity across retries. It must not run a second onboarding-only profile parser or claim that Magi has remembered something when the normal memory pipeline extracted no durable signal. A successful answer consumes the persona's first-contact state and opens that same chat, preventing a duplicate bootstrap greeting. Empty answers stay on the step, while runtime, network, or completion failures preserve the draft and allow a safe retry without duplicating the message.
