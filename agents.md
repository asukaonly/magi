...
5. **Types**: if the request/response schema changed, update its owning production model or serializer and the matching frontend contract. For generated contract families, update the exporter/fixtures and run `scripts/export-frontend-contracts.py` followed by `npm run contracts:generate` in `frontend/`; never hand-edit generated types or validators. Update hand-written client types only for contracts that are still hand-owned. Cover the boundary with focused backend/frontend tests, including rejection of invalid responses.
