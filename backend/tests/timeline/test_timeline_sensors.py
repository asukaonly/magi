"""Tests for the photo-library sensor under the current session-based contract."""
...
class TestSensorIdentity:
    def test_source_item_identity_uses_session_key(self):
        ...
        item = _session_item()
        assert sensor.source_item_identity(item) == "session:2024-03-09:canon-eos-r5:35.66,139.75"

    def test_source_item_identity_falls_back_to_unknown(self):
        sensor = PhotoLibraryTimelineSensor(source_paths=["/photos"])
        assert sensor.source_item_identity({}) == "session:unknown"
    ...
    def test_version_fingerprint_changes_with_session_key(self):
        ...
        assert sensor.source_item_version_fingerprint(item_a) != sensor.source_item_version_fingerprint(item_b)
...
async def test_fetch_item_returns_a_copy_without_scope_enforcement(tmp_path: Path):
    ...
    fetched = await sensor.fetch_item(source_item)
...
async def test_build_output_for_photo_session():
    sensor = PhotoLibraryTimelineSensor(source_paths=["/photos"])
    output = await sensor.build_output(_session_item())
...
async def test_build_output_without_device_or_location():
    sensor = PhotoLibraryTimelineSensor(source_paths=["/photos"])
    output = await sensor.build_output(
        _session_item(
            device_name="",
            device_slug="",
            location_name="",
...
async def test_l2_batch_policy_groups_sessions_by_month():
    sensor = PhotoLibraryTimelineSensor(source_paths=["/photos"])
    output = await sensor.build_output(_session_item(first_capture_ts=1710000000.0, last_capture_ts=1710003600.0))
...
async def test_collect_items_returns_settled_sessions(tmp_path: Path):
    ...
    sensor = PhotoLibraryTimelineSensor(source_paths=[str(photos_dir)], reader=reader, analysis_features=[])
...
async def test_insight_pipeline_enforces_source_edge_whitelist():
    ...
    output = await sensor.build_output(source_item)
    metadata = await sensor.extract_metadata(source_item)
    projection = build_sensor_projection(sensor, output)
    event = build_sensor_timeline_event("evt_photo_whitelist_1", output, projection, metadata)
