...
from magi.awareness import SensorSyncResult as BackendSensorSyncResult
...
from magi_plugin_sdk.sensors import SensorSyncResult as SdkSensorSyncResult


class StubRuntimePaths:
...
def test_backend_sensor_contracts_reexport_sdk_symbols() -> None:
    ...
    assert BackendSensorSyncResult is SdkSensorSyncResult


def test_sensor_sync_context_accepts_runtime_path_protocol() -> None:
    context = SdkSensorSyncContext(
        ...
        limit=100,
        runtime_paths=StubRuntimePaths(),
        plugin_settings={},
    )

    assert context.runtime_paths.plugin_cache_dir("photo-library") == Path("cache") / "photo-library"