...
from magi.api.routers.source_status_projection import (
...
def test_current_source_settings_masks_declared_and_secret_named_values() -> None:
    item = SimpleNamespace(
        fields=[
            ExtensionFieldSpec(
                key="sources.example.access_token",
                type="secret",
                label="Access token",
            ),
            ExtensionFieldSpec(
                key="sources.example.label",
                type="input",
                label="Label",
            ),
        ],
        metadata={
            "activation_flow": {
                "fields": [
                    {
                        "key": "sources.example.client_secret",
                        "type": "secret",
                        "default": "",
                    }
                ]
    ...
    assert _current_source_settings(item, settings) == {
        "sources.example.access_token": "***",
        "sources.example.label": "Example",
        "sources.example.client_secret": "***",
    }


def test_serialize_source_sync_activity_exposes_backfill_range_and_result() -> None:
    activity = _serialize_source_sync_activity(
        {
            "job_id": "job-1",
            "status": "success",
            "payload": {
...
def test_serialize_source_sync_activity_keeps_batch_transition_active() -> None:
    activity = _serialize_source_sync_activity(
        {
            "job_id": "job-2",
            "status": "success",
...
def test_serialize_source_sync_activity_exposes_durable_retry_state() -> None:
    activity = _serialize_source_sync_activity(
        {
            "job_id": "job-3",
            "status": "queued",
