...
_PUBLIC_ROUTE_METHODS: dict[str, dict[str, set[str]]] = {
    ...
    "memory": {
        ...
        "/history-imports/{job_id}/markdown/append": {"POST"},
        "/history-imports/importers": {"GET"},
        "/history-imports/importers/{plugin_id}/{importer_id}/preview": {"POST"},
        "/history-imports": {"GET"},
    ...
    "delivery": {"/events": {"POST"}, "/collector/{connection_id}": {"GET", "POST"}, "/collector/{connection_id}/release": {"POST"}, "/status": {"GET"}, "/retry": {"POST"}, "/discard": {"POST"},
