"""Read-model projection for source status responses."""
...
_SOURCE_INTERNAL_ERROR_MARKERS = ("<Queue at ", "MemoryEvent(", " is bound to a different event loop")
...
def _sanitize_source_error(error: Any) -> str | None:
    ...
    if " is bound to a different event loop" in text:
        return core_i18n.t(
            "sources.sync.loop_mismatch",
            fallback="Source sync failed due to an internal runtime loop mismatch.",
        )
    if any(marker in text for marker in _SOURCE_INTERNAL_ERROR_MARKERS):
        return core_i18n.t(
...
async def build_source_status_payload(
    *,
    runtime_paths: Any,
    manager: Any,
    source_registry: Any,
...
async def _build_source_status(
    ...
    *,
    ...
    if resolved is None:
        raise RuntimeError("Source contribution has no matching connection")
    source = resolved[2]
    ...
    source_settings["enabled"] = source.connection.enabled and source_settings["enabled"]
    capabilities = _resolve_source_capabilities(source, state)
    sync_activity = _serialize_source_sync_activity(latest_sync_job)
    ...
    return {
        ...
        **_source_run_payload(
        ...
        "sync_activity": sync_activity,
        "connection_id": connection_id,
        "connection_display_name": source.connection.display_name,
        "connection_revision": source.connection.revision,
...
async def _load_scheduler_state(
    ...
    *,
    ...
    target_key = build_source_target_key(connection_id, source_name)
    schedule_id = build_source_schedule_id(connection_id, source_name)
    state = await repository.get_target_state(ScheduledTargetType.SOURCE_SYNC, target_key)
    ...
    latest_sync_job = await repository.get_latest_source_sync_job(
        ScheduledTargetType.SOURCE_SYNC,
        target_key,
    )
...
def _serialize_source_sync_activity(
...
def _resolve_source_capabilities(source: Any, state: Any) -> dict[str, Any]:
    supports_pull_sync = bool(getattr(source, "supports_pull_sync", False))
    return {
        "supports_pull_sync": supports_pull_sync,
        "supports_state_flush": bool(getattr(source, "supports_state_flush", False)),
        "running": bool(state.running) if state is not None else False,
        "last_error": (
            _sanitize_source_error(state.last_error)
