...
class TraceQueryTool(Tool):
    ...
    async def execute(
        ...
        resolved_turn_id = requested_turn_id or self._resolve_turn_id(
        ...
        snapshot = trace_service.get_trace_snapshot(user_id=user_id, session_id=session_id, turn_id=resolved_turn_id)
        ...
        tool_calls = self._collect_tool_calls(
            ...
            include_result_json=include_result_json,
        )
        if tool_name_filter and not tool_calls:
            return ToolResult(
                success=False,
                error=f"No tool call named {tool_name_filter} found in turn {resolved_turn_id}",
