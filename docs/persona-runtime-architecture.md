...
Before onboarding starts the full staged pipeline, it uses a short structured LLM call to resolve generation intent. This resolver classifies the input as original, one resolved reference, multiple ambiguous reference candidates, or unknown. It does not generate persona content or verify canon. Any detected fictional, public-person, or private-person reference is editable. Ambiguous references must be confirmed by the user, and the confirmed name, work, version, adaptation mode, expression profile, and explicit constraints become authoritative input to every generation stage. Semantic routing belongs to this structured resolver; code should only validate schema and impossible combinations rather than maintain name dictionaries or phrase rules.

Reference fidelity and expression intensity are independent. An inspired fictional mode creates a new identity without inherited lore; natural fictional mode keeps the fictional identity with ordinary low-performance chat; immersive fictional mode allows more canon texture only under relevant triggers. Public-person modes create a new persona grounded in public-facing traits or expression; they do not turn the reference into a generic assistant, must never impersonate the person, and must not infer private history. Private-person references may use only details supplied by the user.

After the user confirms a fictional or public reference, the staged generator first prepares a structured reference profile from model parametric knowledge. This profile must carry explicit unverified provenance, must not be treated as source-backed evidence, and must never run for original or private-person intents. The profile separates ordinary baseline, judgment patterns, speech rhythm, interaction patterns, signature markers, contrast contexts, version notes, and unknowns. Each generation stage receives only the dimensions it needs, while user-confirmed intent remains authoritative. A later source-backed research layer may replace or correct this prior without changing the generation-stage contracts.
