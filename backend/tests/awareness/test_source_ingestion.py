...
class NotesSource(Source):
    ...
    async def extract_metadata(self, item):
        return SourceOutputMetadata(tags=["notes"], fact_hints=[{
...
async def setup_runtime(tmp_path, sources):
    ...
    gateway = SourceIngestionGateway(event_bus=bus, memory_committer=SourceEventCommitter(unified_memory=memory))
    registry = Mock()
    registry.resolve_source.side_effect = lambda source_type, connection_id: (
...
async def sync(contrib, connection_id="account-a", scheduler_cursor=None):
    return await contrib._run_source_sync(
...
async def test_scheduler_commits_real_l1_and_connection_identity_without_changing_source_type(tmp_path):
    sources = {name: NotesSource(owner(name), tmp_path) for name in ("account-a", "account-b")}
    memory, contrib, bus = await setup_runtime(tmp_path, sources)
    try:
        ...
        assert source_object_identity("account-a", "notes.source", "one") in {row["source_item_id"] for row in stored}
...
async def test_partial_failure_retries_original_batch_before_advancing_cursor(tmp_path):
    ...
    source.fail_object = "two"
    memory, contrib, _ = await setup_runtime(tmp_path, {"account-a": source})
    try:
        with pytest.raises(RuntimeError, match="Transient"):
            await sync(contrib)
        checkpoint = await contrib.source_store.checkpoint(owner(), "notes.source", "notes")
        assert checkpoint.cursor is None
        assert len(await events(memory)) == 1
        source.fail_object = None
        source.batch = SourceChangeBatch(changes=[], next_cursor="must-not-collect")
        ...
        assert await contrib.source_store.pending(await contrib.source_store.checkpoint(owner(), "notes.source", "notes")) is None
    finally:
        await memory.shutdown()
...
async def test_new_object_revision_reaches_l1_and_deletion_does_not_forget_history(tmp_path):
    source = NotesSource(owner(), tmp_path)
    source.batch = SourceChangeBatch(changes=[source.batch.changes[0]], next_cursor="one")
    memory, contrib, _ = await setup_runtime(tmp_path, {"account-a": source})
    try:
        await sync(contrib)
        source.batch = SourceChangeBatch(changes=[SourceChange(object_id="one", version="v2", payload={"id": "one", "text": "Revised note"})], next_cursor="two")
        await sync(contrib)
        assert len(await events(memory)) == 2
        source.batch = SourceChangeBatch(changes=[SourceChange(object_id="one", version="v3", operation="delete")], next_cursor="three")
        await sync(contrib)
        assert len(await events(memory)) == 2
        checkpoint = await contrib.source_store.checkpoint(owner(), "notes.source", "notes")
...
@pytest.mark.asyncio
async def test_untyped_source_result_is_rejected_without_checkpoint_progress(tmp_path):
    source = NotesSource(owner(), tmp_path)
    source.batch = SimpleNamespace(items=[], next_cursor="invalid")
    memory, contrib, _ = await setup_runtime(tmp_path, {"account-a": source})
    try:
        with pytest.raises(TypeError, match="SourceChangeBatch"):
            await sync(contrib)
        assert (await contrib.source_store.checkpoint(owner(), "notes.source", "notes")).cursor is None
    finally:
        await memory.shutdown()
...
async def test_scoped_output_resource_is_imported_and_revoked_with_source_delete(tmp_path):
    source = NotesSource(owner(), tmp_path)
    source.batch = SourceChangeBatch(changes=[source.batch.changes[0]], next_cursor="one")
    thumbnail = source.context.resources_dir / "thumbnail.txt"
    thumbnail.write_text("private content")
    build = source.build_output
    ...
    source.build_output = with_resource
    ...
    try:
        ...
        source.batch = SourceChangeBatch(changes=[SourceChange(object_id="one", version="v2", operation="delete")], next_cursor="two")
        await sync(contrib)
...
async def test_push_timeline_uses_same_source_acceptance_and_real_l1(tmp_path, monkeypatch):
    ...
    source = NotesSource(owner(), tmp_path)
    memory, contrib, bus = await setup_runtime(tmp_path, {"account-a": source})
    contrib._source_registry.resolve_source.side_effect = lambda source_type, connection_id: (
        "notes", "notes.source", source, SourceSpec(source_id="notes.source", display_name="Notes", domain="timeline")
    )
    monkeypatch.setattr(timeline, "get_runtime_paths", lambda: SimpleNamespace(runtime_dir=tmp_path))
    handler = timeline.build_timeline_handler(None, memory, source_registry=contrib._source_registry, plugin_manager=contrib._plugin_manager, ingestion_gateway=contrib._ingestion_gateway)
    try:
        result = await handler({"source_type": "notes", "connection_id": "account-a", "source_change": source.batch.changes[0].model_dump()})
...
@pytest.mark.asyncio
async def test_push_only_source_is_queued_without_worker_reentry_then_committed(tmp_path):
    source = NotesSource(owner(), tmp_path)
    source.supports_pull_sync = False
    memory, contrib, _ = await setup_runtime(tmp_path, {"account-a": source})
    contrib._scheduler_service.schedule_once = AsyncMock()
    payload = {"source_type": "notes", "connection_id": "account-a", "source_change": source.batch.changes[0].model_dump()}
    try:
...
async def test_connection_readiness_ignores_other_account_and_deleted_l1(tmp_path):
    ...
    sources = {name: NotesSource(owner(name), tmp_path) for name in ("account-a", "account-b")}
    memory, contrib, _ = await setup_runtime(tmp_path, sources)
    try:
        await sync(contrib, "account-a")
        await sync(contrib, "account-b")
