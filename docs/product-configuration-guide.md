...
The history-import path is a one-shot transfer rather than a continuous activity source. The host owns the Markdown picker, preview, per-file inclusion, identity confirmation, progress, retry, deletion, and memory handoff. It accepts `.md` and `.markdown` files or folders, presents the task as importing journals, notes, or conversations rather than as a format-first workflow, detects chat-shaped records versus personal writing, and supports a bounded per-file preview before any memory write. The preview renders Markdown structure such as headings, lists, quotations, and code while leaving embedded HTML inert. The selected file subset is durable across navigation and refresh. The selection list stays compact, offers select-all and invert-selection actions, and treats choosing a personal-writing file as the authorship confirmation for that file instead of asking for a second global checkbox. Files dominated by quotations, saved articles, or other people's writing must be left unselected. Ordinary personal Markdown uses one file as one authored document: headings, date-like headings, and long sections remain structure inside that document and never create additional import records. Chat-shaped Markdown remains message-grained because speaker identity and conversation order are semantically meaningful. Long-document vector indexing may create internal retrieval chunks later, but every chunk remains attached to the same parent event and is collapsed back to that event for product-facing recall; chunks are not presented as imported content or separate memories. Users must identify their own speaker names for conversations. Messages from other participants are retained only as conversation context and are not treated as claims about the user.
