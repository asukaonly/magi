...
and shell-quoted. Packaged commands use the actual executable path, including
collector guidance passed through the Python process, without relying on `PATH`.
A missing identity file with a verified active background owner
...
- persona selection defaults to the lightweight trial chat and offers an optional read-only detail view sourced from the same preset or generated persona config; switching views must preserve the trial transcript, and revealing deep relationship layers still requires explicit spoiler confirmation. After five user turns, trial chat shows non-blocking guidance toward the next step while keeping the composer available
...
- unfinished custom descriptions, confirmed references, generation job IDs, generated drafts, and the visible persona subview must survive onboarding navigation and refresh; retaining a custom draft must not replace the selected persona preview when the user navigates backward and forward, and generation start requests use a stable request ID so a network retry cannot launch duplicate jobs
...
Large conversation lists remain complete and user-selectable within the bounded
preview contract, while the screen renders them in pages instead of mounting
the entire list at once. The product must fail explicitly when an export exceeds
that contract; it must never present an adapter-chosen recent subset as a
complete import.
...
Confirmation prepares a bounded recent raw slice for first-chat context without waiting for durable conclusions. The first-contact opening may use at most one low-sensitivity detail from this explicitly selected user writing, must treat archive text as data rather than instructions, and must not mention files, imports, memory, or sources. Thin, private, or ambiguous material is ignored rather than forcing personalization. The complete selection is then stored and submitted to the ordinary memory pipeline. Reader-facing progress distinguishes original-text persistence from durable L2 queue handoff: the product renders them as two labeled stages with independent counts and never presents source-text saving as an aggregate import percentage. An import can report that source text is saved while some records are waiting or need a retry before memory processing accepts them. Neither state claims that assertions, graph relationships, or portraits have already been derived. A document has only one document-level time: common frontmatter date fields, a dated filename, or file time as an explicitly approximate fallback. Body headings are content, not a document segmentation or timestamp contract.
