...
@pytest.mark.asyncio
async def test_l4_updates_existing_task_preference(tmp_path) -> None:
    store = L4ProceduralMemoryStore(
        db_path=str(tmp_path / "memory.db"),
        vector_enabled=False,
    )
    ...
    first_skill_id = await store.record_task_preference(
        user_id="u1",
        persona_id="seven",
        task_category="coding",
        preference="改代码前先讲完成标准",
    ...
    second_skill_id = await store.record_task_preference(
        user_id="u1",
        persona_id="seven",
        task_category="coding",
        preference="改代码前先讲完成标准",
        polarity="prefer",
        evidence_text="再确认一遍完成标准。",
        confidence=0.8,
        turn_id="turn-2",
    ...
    assert second_skill_id == first_skill_id
    assert preferences == [
        {
