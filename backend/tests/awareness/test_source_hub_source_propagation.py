"""Regression: SourceHub must propagate ``source`` from the published event
into the source_event payload — otherwise ``UserMessagePayload.from_dict``
...
correctly published it on ``event.data["source"]``, but ``SourceHub.
_on_user_message`` copied a hand-picked subset of fields onto
``source_event.payload`` — ``source`` was not in that subset.
"""
...
@pytest.mark.asyncio
async def test_on_user_message_propagates_source_to_source_event_payload():
    """Phase H+1 plumbing: the ``source`` field must reach the source_event
    ...
    assert len(batch) == 1
    source_event = batch[0]
    assert source_event.payload.get("source") == "weixin"
    assert source_event.delivery_attempt_no == 3
    assert source_event.runtime_command_id == 41
...
async def test_on_user_message_defaults_source_to_api_when_event_lacks_one():
    ...
    must still produce a valid source_event — default to "api" so
