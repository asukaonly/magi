"""Sensor sync job admission, claiming, and recovery."""
...
from .contracts import SensorSyncEnqueueResult, _SensorJobRepositoryHost


class _SensorSyncJobAdmissionMixin:
    async def enqueue_sensor_sync_execution(
        self,
        *,
        schedule: ScheduleDefinition,
        manual: bool,
        started_at: float,
    ) -> Optional[SensorSyncEnqueueResult]:
        ...
        try:
            ...
            return None
        return SensorSyncEnqueueResult(
            job_id=job_id,
    ...
    async def claim_next_sensor_sync_job(self, *, claimed_by: str) -> Optional[dict[str, object]]:
        ...
        async with host._connect() as db:
            ...
            if target_update.rowcount != 1:
                await db.rollback()
                raise RuntimeError(
                    "Sensor sync target state not found while claiming job: "
                    f"{row['target_type']}:{row['target_key']}"
                )
            await db.commit()
        return await host.get_sensor_sync_job(job_id)

    async def recover_running_sensor_sync_jobs(self) -> int:
        ...
        async with host._connect() as db:
            ...
            await db.execute(
                ...
                UPDATE target_state
                SET running = 0,
                    last_error = 'SENSOR_SYNC_EXECUTOR_RESTARTED',
                    updated_at = ?
                WHERE EXISTS (
                    SELECT 1
                    FROM sensor_sync_jobs AS job
            ...
            cursor = await db.execute(
                ...
                SET status = 'queued',
                    ...
                    finished_at = NULL,
                    next_attempt_at = ?,
                    error = 'SENSOR_SYNC_EXECUTOR_RESTARTED'
