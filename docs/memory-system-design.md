...
- Phase 1 extracts current-batch entities, facts, and candidate observations from admitted events, using source-owned hints and extraction-profile instructions as anchors. The host then assigns each retained fact a deterministic claim reference and verifies its quoted evidence against the exact current events. Missing, out-of-batch, or unmatchable support triggers one schema-correction retry and is rejected if the retry remains invalid; it is never expanded to the whole batch.
