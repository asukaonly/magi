...
def resolve_llm_profile(
    ...
    if provider_settings is not None:
        ...
        if resolved_model_meta is None:
            resolved_model_meta = next(
                (model for model in resolved_catalog.image_generation_models if model.id.lower() == lowered_model),
                None,
            )
    model_meta = resolved_model_meta or find_chat_model_meta(registry, provider_name, model_name)

    if model_meta is not None and hasattr(model_meta, "capabilities"):
    ...
    elif find_image_generation_model_meta(registry, provider_name, model_name) is not None:
        image_meta = find_image_generation_model_meta(registry, provider_name, model_name)
        capabilities = LLMCapabilitiesSettings(
            vision=False,
            image_output=True,
            tool_calling=False,
        ...
        provider_options = dict(image_meta.provider_options_example if image_meta is not None else {})
    else:
