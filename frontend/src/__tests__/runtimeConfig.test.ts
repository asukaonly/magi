...
const started = (overrides = {}) => ({ ok: true, connectionGeneration: 1, baseUrl: 'http://127.0.0.1:19080/api', sessionToken: 'a'.repeat(64), serverId, profileId: 'local', mode: 'local', dataEpoch: serverId, contentEpoch: serverId, expiresAtMs: null, localServicePid: 123, ...overrides });
...
describe('center runtime bootstrap', () => {
  ...
  it('deduplicates bootstrap and accepts readiness from the authenticated native probe', async () => {
    invokeMock.mockImplementation(async (command) => command === 'connect_active_profile' ? started() : { connectionGeneration: 1, ready: true, phase: 'ready' });
  ...
  it('retires the old connection before asking app bootstrap to reconnect', async () => {
    invokeMock.mockImplementation(async (command) => command === 'connect_active_profile'
      ? started() : { connectionGeneration: 1, ready: true, phase: 'ready' });
  ...
  it('reloads credentials if the native generation changes while readiness is polled', async () => {
    invokeMock.mockResolvedValueOnce(started())
      .mockResolvedValueOnce({ ready: true, phase: 'ready', connectionGeneration: 2 })
      .mockResolvedValueOnce(started({ connectionGeneration: 2, sessionToken: 'b'.repeat(64), localServicePid: 456 }))
      .mockResolvedValueOnce({ ready: true, phase: 'ready', connectionGeneration: 2 });
    await expect(initializeRuntime()).resolves.toMatchObject({
      connectionGeneration: 2, sessionToken: 'b'.repeat(64), localServicePid: 456,
    });
  ...
  it('allows maintenance recovery to show without requiring ordinary runtime readiness', async () => {
    invokeMock.mockImplementation(async (command) => command === 'connect_active_profile' ? started() : { connectionGeneration: 1, ready: true, phase: 'recovering_maintenance' });
    const phases: string[] = [];
