...
Builtin personas are source-controlled presets. Startup synchronizes changed bundles through the initialization-state fingerprint; explicit seed installation can also synchronize them. Registry listing is read-only: a connected desktop refreshing its list must not rewrite another editor's configuration. Unchanged seed rows retain their version. Custom personas remain registry-owned and are not overwritten by seed synchronization.

Persona configuration and its reference dossier share the registry row's monotonic `updated_at` version. Public update and delete require the displayed `expected_updated_at` (428 when absent, 409 when stale). SQLite transactions check that version and return configuration plus provenance from the same committed snapshot. The editor retains its original version while dirty, advances it only after its own confirmed write, and provides explicit reload after a conflict. Read-only center refreshes retain the selected persona and unsaved content; delayed reads cannot replace a confirmed save. Create retries reuse the same persona ID and do not rewrite an existing record or dossier, including after a later edit. Soft-deleted IDs remain unavailable for reuse.
...
- the fixed `surface` layer, plus deeper relationship layers only when the user's input or confirmed reference supports them

Generation prompts should explicitly say that ordinary, low-performance replies are valid and desirable for most turns.

Before onboarding starts the full staged pipeline, it uses a short structured LLM call to resolve generation intent. This resolver classifies the input as original, one resolved reference, multiple ambiguous reference candidates, or unknown. It does not generate persona content or verify canon. Any detected fictional, public-person, or private-person reference is editable. Ambiguous references must be confirmed by the user, and the confirmed name, work, version, fidelity level, expression level, research preference, supplied reference URLs, and explicit constraints become authoritative input to every generation stage. Semantic routing belongs to this structured resolver; code should only validate schema and impossible combinations rather than maintain name dictionaries or phrase rules.

Reference fidelity and expression intensity are independent and use one contract for fictional and public references. `traits` creates a new identity from broad behavioral tendencies, `natural` keeps a recognizable public or fictional presence with ordinary low-performance chat, and `faithful` permits more verified texture only under relevant triggers. `low`, `balanced`, and `high_contextual` expression control how often that texture becomes visible. A public-person reference must never claim to be the real person or infer private history. Private-person references are restricted to `traits`, use only details supplied by the user, and always disable web research.

After the user confirms a fictional or public reference, the staged generator first prepares a structured reference profile from model parametric knowledge. This profile carries explicit unverified provenance and never runs for original or private-person intents. A deterministic policy then selects no research, identity-only verification, representative research, or full research from fidelity, identity confidence and ambiguity, prior coverage, volatility, user URLs, and explicit network preference. When network research is enabled, `natural` always requires representative behavioral research even if the model prior looks complete; a fluent model prior is not evidence that a recognizable public expression is accurate. `traits` may use no or identity-only research when the prior and identity are sufficient, while `faithful` requires full research. The same policy applies to all public and fictional references; it must not grow identity-category exception branches.

Network research uses the shared tool invocation boundary for discovery and safe page fetching. Source pages are untrusted input. The extraction stage may produce only bounded behavioral claims linked to source IDs, plus contradictions and unknowns; unsupported or invalid citations are discarded. Verified evidence can replace the matching parametric-prior dimension, while uncovered dimensions remain explicitly unverified. `faithful` generation requires a verified identity and sufficient multi-source coverage and fails instead of downgrading. Other fidelity levels may continue with a visible `model_prior`, `unavailable`, or `insufficient` status.

The resulting dossier is part of the generation result and is persisted by stable `persona_id` alongside the persona registry record. It contains the canonical reference fingerprint, grounding and identity status, source metadata, evidence, coverage, volatility, contradictions, and unknowns. Runtime persona planning consumes only the generated persona configuration; the dossier remains provenance for review and regeneration rather than being appended to every chat prompt.

Generation may receive an existing draft config. In that case, it should preserve explicit user-authored fields unless the user asks to replace them, then fill missing target-schema fields. Post-generation normalization may complete required runtime surfaces such as core registers, quiet hours, signature triggers, the fixed `surface` layer, examples, and bootstrap defaults so generated personas are immediately editable and runnable. It must not invent non-surface relationship layers for sparse input. The generator must not let the model customize `surface`; supported relationship-depth behavior belongs in non-surface layers such as `crack` and `revealed`.

The UI should show staged generation feedback while the request is running through a lightweight generation job protocol. Onboarding first calls `POST /api/personality/generation-intents/resolve`, then starts confirmed generation with `POST /api/personality/generation-jobs` and polls `GET /api/personality/generation-jobs/{job_id}` until the job reaches `completed` or `failed`. This is intentionally long polling rather than WebSocket because persona generation is a short-lived settings/onboarding workflow and does not need a persistent realtime channel. Generation requests carry stable draft and request IDs; replaying the same request ID returns the existing job rather than launching duplicate model work. Only an explicit server-side `failed` state is terminal: a timeout, lost start response, or polling error retains the request ID and any known job ID so reload or retry resumes the same generation.

Onboarding persona adjustment uses a separate `POST /api/personality/adjust` request rather than treating chat text as an implicit config mutation. The adjustment service returns a scoped patch and host code filters allowed top-level fields before merging it into the current valid draft. Voice and expression requests cannot rename the persona or replace its identity. The client increments the draft revision, keeps the previous preview answer as a superseded comparison, and re-runs the latest user turn with the new config. Only the accepted answer variant is included in later preview history.
...
Assistant-role framing and configuration-language leakage are final acceptance failures unless the user explicitly requested an assistant role. The integration stage receives any pre-integration findings, then the same deterministic checks run again after integration. A remaining finding receives one focused correction pass and one final recheck; if it still remains, generation fails rather than returning a known-bad persona as successful.
