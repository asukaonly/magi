...
Magi 是一个本地优先的桌面 Agent：它可以对话、调用工具、执行后台任务，并把长期上下文交还给你控制。

多数 Agent 只能看到当前这次输入。Magi 面向的是会持续几天、几周甚至几个月的工作、习惯、偏好和决定。只要你授权，它会把对话、日历、浏览记录、Git 活动、音乐、照片和其他本地来源里的有用信号整理成时间线与可检查的长期记忆，让之后的回答和行动建立在真实发生过的上下文之上。

## 为什么时间重要

时间是很多 AI 工作流里缺失的上下文。

一个助手可以在单次窗口里回答问题；一个真正长期可用的 Agent 还需要理解什么变了、什么反复出现、你纠正过什么，以及什么应该被忘掉。Magi 把时间当成产品的一部分：事件成为时间线，反复出现的模式成为记忆，而所有沉淀下来的记忆都可以被你检查。

- **它首先是桌面 Agent，不只是聊天界面** — Magi 可以回答、调用工具、处理权限、中断恢复，并把长任务放到后台继续运行。
- **记忆会随着时间积累** — 记忆链路面向事实、偏好、跨会话模式和变化更新，在 LongMemEval 上达到 **87.2% accuracy**。
- **时间线可以被追问** — 对话和授权插件事件会进入可搜索、可回看的时间线，而不是散落成一堆日志。
- **默认可检查** — 你可以看到 Magi 记住了什么，修正不可靠或错误的推断，删除不想保留的内容。
- **本地优先** — 应用和运行数据默认保存在你的机器上（`~/.magi`），只有模型调用或你配置的动作才会外发数据。
- **互动更连续** — 人格档案、关系深度和自然回复节奏，让 Magi 不像一个每次都从零开始的命令框。

## 时间怎么成为上下文

Magi 把你授权的本地信号接入 Agent 运行时，并形成一条你可以检查和控制的循环。
