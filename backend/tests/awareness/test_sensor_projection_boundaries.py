...
def test_sensor_event_projection_is_owned_by_consumers() -> None:
    legacy_mixed_projection = SRC_ROOT / "awareness" / "sensor_memory_projection.py"
    memory_projection = SRC_ROOT / "memory" / "sensor_event_projection.py"
    timeline_projection = SRC_ROOT / "timeline" / "sensor_event_projection.py"
    ...
    violations = {
        ...
        if (
            imports := [
                ...
                if name == "magi.awareness.sensor_memory_projection"
            ]
        )
    }
