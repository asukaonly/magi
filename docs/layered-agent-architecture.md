...
two contracts, both passing. Plugin system assembly lives in
`bootstrap/plugin_system.py`; skill registration belongs to `skills`, and source
content coordination belongs to `awareness`. The plugin operation registry
receives an invocation callback from the composition root, backed by the agent
tool invocation service and its required effect ledger. Registration-only hosts
without that callback reject execution; they never bypass governance. The
existing two-entry ignore list must not be expanded.
...
- Console prompts use `clap` and `cliclack` in `server/`; the console API adapter acquires short-lived operator authorization from the same-account management socket and calls the existing authenticated product APIs. Rust does not write business YAML or persona databases. Interactive foreground setup holds a private lifetime pipe to the ordinary headless owner; loss of the console closes that pipe and drains its service. Background setup delegates lifetime to the macOS login service. Explicit `run` remains non-interactive.
- `crates/magi-service-contract/` owns launch configuration, bootstrap messages and the protocol version without pulling in runtime, HTTP or database libraries. The desktop may depend on this contract, `magi-platform`, and the producer-owned `magi-delivery` outbox, but must not import service implementations or read center databases. `magi-delivery` owns only its private client queue; the Python ingress processor owns business consumption and retry-safe handler registration. Durable center-clear markers belong to `magi-server-runtime`, not the platform library. `scripts/check-rust-boundaries.py` enforces the workspace dependency direction.
...
- the Python agent, plugins, and business layers must not receive, persist, log, or place gateway credentials in resource URLs. The independent Python collector transport is a device-side authentication owner and stores only its scoped collector credential; it does not bootstrap the agent or expose that credential to its plugin context
