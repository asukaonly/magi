...
async function generate(domain, names) {
  const contractUrl = new URL(`../../contracts/api/frontend-${domain}.json`, import.meta.url);
  ...
  const declarations = header +
    `import type { components } from './${domain}-types';\n` +
    names.map((name) => 'export declare function validate' + name +
      "(value: unknown): value is components['schemas']['" + name + "'];").join('\n') + '\n';
  const outputs = { [domain + '-types.ts']: types, [domain + '-validators.js']: validators, [domain + '-validators.d.ts']: declarations };
...
await generate('config', ['ConfigResponse', 'OnboardingStatusResponse', 'OnboardingTemplateResponse', 'ToolConfigResponse', 'ToolsListResponse']);
