"""Phase 7 of C: SourceStateUpdateSubscriber persists fingerprints."""
...
from magi.awareness.subscribers.source_state_update_subscriber import SourceStateUpdateSubscriber


def _payload(*, fingerprint, source_id="x"):
    return SourceEventEmitted(
        source_name=source_id, payload={}, context=TaskContext(None, None, None, "u"),
...
async def test_persists_fingerprint(fake_bus, fake_store):
    sub = SourceStateUpdateSubscriber(event_bus=fake_bus, source_state_writer=fake_store)
    await sub.start()
    payload = _payload(fingerprint="fp-1", source_id="screen_time")
...
async def test_handler_failure_swallowed(fake_bus, fake_store):
    ...
    await sub.start()
    await sub._on_event(
        Event(type=EventTypes.SOURCE_EVENT_EMITTED, data=_payload(fingerprint="fp"), event_id="e")
    )
