...
async def test_read_chat_attachment_reads_text_attachment_by_id(tmp_path: Path) -> None:
    ...
    try:
        ...
        tool = ReadChatAttachmentTool(read_service_factory=lambda: _FakeReadService(payload))

        result = await tool.execute({"attachment_id": "att-1", "limit": 8}, _context())
...
async def test_read_chat_attachment_returns_image_metadata_without_path(tmp_path: Path) -> None:
    ...
    try:
        ...
        result = await tool.execute({"attachment_id": "att-1"}, _context())
...
async def test_read_chat_attachment_requires_session_id() -> None:
    tool = ReadChatAttachmentTool(read_service_factory=lambda: _FakeReadService(None))

    result = await tool.execute(
        {"attachment_id": "att-1"},
        ToolExecutionContext(agent_id="chat-agent", env_vars={"user_id": "local_user"}),
    )
