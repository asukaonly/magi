...
from benchmark.locomo.run_all import main, parse_args, run_locomo_pipeline
...
def test_run_all_query_only_skips_replay_and_reuses_existing_run(tmp_path) -> None:
    ...
    def replay_runner(**kwargs):
        raise AssertionError(f"query-only should not replay records: {kwargs}")

    def query_runner(**kwargs):
        calls.append(("query", kwargs["qa_limit"], kwargs["backend_url"]))
        run_dir = tmp_path / "locomo" / "existing-run"
        run_dir.mkdir(parents=True, exist_ok=True)
        (run_dir / "summary.json").write_text('{"total_questions": 1}\n', encoding="utf-8")
        (run_dir / "predictions_with_trace.jsonl").write_text(
            (
                ...
                '"question":"What did Caroline join?","expected_answer":"support group",'
                '"evidence":["D1:1"],"answer_session_ids":["session_1"],'
                '"hypothesis":"support group","retrieved_session_ids":["session_1"],'
    ...
    assert calls == [
        ("query", 1, "http://127.0.0.1:8000"),
        ("judge", 1, "http://127.0.0.1:8000"),
    ]
    assert summary["run_id"] == "existing-run"
    assert summary["summary"]["total_questions"] == 1
    assert summary["llm_judge"]["llm_judge_score"] == 1.0
    assert (tmp_path / "locomo" / "existing-run" / "error_report_summary.json").exists()


def test_parse_args_accepts_query_only() -> None:
    args = parse_args(
        [
            "--output-root",
            "/tmp/out",
...
def test_main_query_only_requires_run_id(tmp_path) -> None:
    with pytest.raises(SystemExit, match="--query-only requires --run-id"):
        main(["--output-root", str(tmp_path), "--query-only"])
