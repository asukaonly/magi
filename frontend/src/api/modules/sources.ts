...
/** Honest memory-readiness signal for one source (see backend GET /sources/{source}/memory-readiness). */
...
export interface SourceSyncActivity {
  ...
  backfill_scope?: SourceBackfillScope | null;
...
export interface SourceStatusItem {
  ...
  sync_activity?: SourceSyncActivity | null;
}
...
export interface SourceAuthorizationResponse {
  authorized: boolean;
...
export interface SourceTodaySummaryEntry {
  source_name: string;
...
export interface SourceTodaySummaryResponse {
  date: string;
  weekday: number;
  sources: SourceTodaySummaryEntry[];
}

export type SourceSyncMode = 'latest' | 'backfill';
export type SourceBackfillScope = 'last_7_days' | 'last_30_days' | 'full' | 'custom';

export interface SourceSyncRequestOptions {
  firstContext?: boolean;
  mode?: SourceSyncMode;
  backfillScope?: SourceBackfillScope;
...
export interface SourceSyncResponse {
  ...
  command_id?: number;
  mode?: SourceSyncMode;
  backfill_scope?: SourceBackfillScope;
...
export const sourcesApi = {
  ...
  getTodaySummary: async (day?: string): Promise<SourceTodaySummaryResponse> => {
    const response = await api.get<SourceTodaySummaryResponse>(
      '/sources/today-summary',
  ...
  getMemoryReadiness: async (
    ...
    const response = await api.get<MemoryReadinessResponse>(
      `/sources/${encodeURIComponent(sourceName)}/memory-readiness`,
  ...
  requestSync: async (
    ...
    const response = await api.post<SourceSyncResponse>(
      `/sources/${encodeURIComponent(sourceName)}/sync`,
      payload,
  ...
  requestStateFlush: async (sourceName: string, connectionId: string): Promise<{ queued: boolean; source_name: string }> => {
    const response = await api.post<{ queued: boolean; source_name: string }>(`/sources/${encodeURIComponent(sourceName)}/flush-state`, {}, { params: { connection_id: connectionId } });
    return unwrapGatewayPayload(response);
  },
  ...
  ): Promise<SourceAuthorizationResponse> => {
    const response = await api.post<SourceAuthorizationResponse>(
      `/sources/${encodeURIComponent(sourceName)}/authorize`,
