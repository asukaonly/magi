...
async def test_memory_layers_create_dedicated_vector_tables(tmp_path):
    ...
    l1_store = L1EventStore(db_path=str(l1_db), embedding_service=embedding_service)
    l3_store = L3SummaryStore(db_path=str(l3_db), embedding_service=embedding_service)
    l4_store = L4ProceduralMemoryStore(db_path=str(l4_db), embedding_service=embedding_service)
...
async def test_memory_layers_create_sqlite_vec_virtual_tables_on_insert(tmp_path):
    ...
    l1_store = L1EventStore(db_path=str(l1_db), embedding_service=embedding_service)
    l3_store = L3SummaryStore(db_path=str(shared_db), embedding_service=embedding_service)
    l4_store = L4ProceduralMemoryStore(db_path=str(shared_db), embedding_service=embedding_service)
