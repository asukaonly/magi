...
Reference fidelity and expression intensity are independent. An inspired fictional mode creates a new identity without inherited lore; natural fictional mode keeps the fictional identity with ordinary low-performance chat; immersive fictional mode allows more canon texture only under relevant triggers. Public-person modes always create an inspired assistant and may use only public-facing traits or expression. They must never impersonate the person or infer private history. Private-person references may use only details supplied by the user.
