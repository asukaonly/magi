...
The history-import path is a one-shot transfer rather than a continuous activity source. The host owns the Markdown picker, preview, identity confirmation, progress, retry, deletion, and memory handoff. It accepts `.md` and `.markdown` files or folders, detects chat-shaped records versus personal writing, shows representative content before any memory write, and requires the user to identify their own speaker names or confirm authorship. Messages from other participants are retained only as conversation context and are not treated as claims about the user. Confirmation prepares a recent raw slice for first-chat context without waiting for durable conclusions. It starts with up to 200 recent records and may expand in bounded increments to at most 500 when needed to find roughly 30 meaningful user-authored records. The complete selection is then stored and submitted to the ordinary memory pipeline oldest-first within each source session. Missing source timestamps preserve file order and remain explicitly approximate rather than becoming invented exact times. Import jobs are durable and idempotent for the same selected content, resume after interruption, and are removed together with their imported memory when the user deletes the import or clears all memory.
