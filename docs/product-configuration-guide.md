...
- systems using trusted Clash, Surge, or sing-box TUN fake-IP DNS may explicitly allow the RFC 2544 benchmark range (`198.18.0.0/15`) for hostname resolution without enabling general private-network access; literal benchmark-range URLs, RFC 1918 addresses, loopback, link-local, and metadata targets remain blocked, and first-run persona reference research should offer an inline enable-and-retry action when this exact compatibility problem is detected
