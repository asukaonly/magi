...
def normalize_phase1_entity_contract(
    ...
    for index, raw_entity in enumerate(raw_entities):
        ...
        if entity.get("referent_kind") != "entity":
            rejected_count += 1
            rejected_non_entity_count += 1
            normalizations.append(f"entities[{index}]: dropped candidate (non-entity or unknown referent)")
            continue
        if not valid_entity_name(surface) or not valid_entity_name(str(entity["normalized_name"])):
            rejected_count += 1
            normalizations.append(f"entities[{index}]: dropped candidate (invalid name shape)")
            continue
    ...
    if rejected_non_entity_count:
        diagnostics["rejected_non_entity_count"] = rejected_non_entity_count
