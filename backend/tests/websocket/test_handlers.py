...
@pytest.mark.asyncio
async def test_handle_send_message_passes_attachments_and_workspace_path(monkeypatch: pytest.MonkeyPatch) -> None:
    captured: dict[str, object] = {}
    ...
    assert response["type"] == "message_sent"
    assert captured["attachments"] == [{"kind": "pdf", "attachment_id": "att-1"}]
