...
* P1 (ADR-0005) removed fields the router emitted but no consumer read:
  ``capabilities`` / ``risky_tools`` / ``needs_workspace`` / ``needs_external``
  / ``background_hint`` / ``effort`` / ``confidence`` / ``memory_layer``.
  ``complexity`` is retained (still a required field threaded through every
  construction site); difficulty is carried by ``thinking_depth``.
"""
