...
import { MEMORY_PRIMARY_ACTION_CLASS } from '@/components/memory/memoryActionStyles';
 ...
 * manual-entries channel (user_authored); the fact lands in the review queue
 * once L2 extraction finishes. */
export const PortraitAddFactRow = ({ onSubmitted }: PortraitAddFactRowProps) => {
  ...
  return (
    <section data-testid="portrait-add-fact">
      <div className="flex items-center gap-3">
        <input
          ...
          className="h-9 min-w-0 flex-1 rounded-mem-sm border border-[hsl(var(--memory-border)/0.5)] bg-transparent px-3 text-sm text-[hsl(var(--memory-title))] outline-none transition-colors placeholder:text-[hsl(var(--memory-muted))] focus:border-[hsl(var(--memory-accent)/0.5)] focus:ring-2 focus:ring-[hsl(var(--memory-accent)/0.14)]"
        />
        <Button
          type="button"
          size="sm"
