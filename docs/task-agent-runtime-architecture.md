...
`ChatHistoryService` owns the runtime bridge from durable summary state into prompt history. Prompt assembly loads the complete durable prompt transcript without a fixed message-count ceiling. When an active `token_budget` summary exists for a session, the service loads `session_origin` and `summary_text`, trims raw prompt history from `first_kept_message_id`, and passes the complete unsummarized tail through `ChatRuntimeContext`. Display-history pagination remains a separate read concern and never defines model context. Direct LLM and function-calling execution both feed that summary context into prompt message assembly before appending the current user turn.
