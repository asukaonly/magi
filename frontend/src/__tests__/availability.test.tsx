...
describe('useAvailability', () => {
  ...
  it('returns entries after fetch resolves', async () => {
    mockedGet.mockResolvedValue({
      entries: [
        {
          plugin_id: 'chrome-history',
    ...
    const { result } = renderHook(() => useAvailability(['chrome-history']));

    expect(result.current.loading).toBe(true);
    await waitFor(() => expect(result.current.loading).toBe(false));
    expect(result.current.entries).toHaveLength(1);
    expect(result.current.byId['chrome-history'].available).toBe(true);
  });

  it('exposes refresh() that re-fetches', async () => {
    // First fetch returns empty
    mockedGet.mockResolvedValueOnce({ entries: [] } as any);
    // Refresh returns invalidation result
    mockedPost.mockResolvedValue({
      invalidated_plugin_ids: [],
    } as any);
    // Second fetch returns one entry
    ...
    expect(result.current.entries[0].plugin_id).toBe('chrome-history');
  });
});
