...
async def test_l1_store_uses_sqlite_vec_for_semantic_search(tmp_path):
    ...
    store = L1EventStore(db_path=str(tmp_path / "l1.db"), embedding_service=FakeEmbeddingService())
...
async def test_l3_store_uses_sqlite_vec_for_semantic_search(tmp_path):
    ...
    store = L3SummaryStore(db_path=str(tmp_path / "memory.db"), embedding_service=FakeEmbeddingService())
    await store.initialize()
...
async def test_l4_store_uses_sqlite_vec_for_semantic_search(tmp_path):
    ...
    store = L4ProceduralMemoryStore(db_path=str(tmp_path / "memory.db"), embedding_service=FakeEmbeddingService())
    await store.initialize()
