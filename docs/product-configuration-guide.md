...
- a dedicated first-context step that gives equal weight to answering one lightweight personal or ordinary-life question, importing past written or chat records, or connecting optional user-approved activity sources
...
The history-import path is a one-shot transfer rather than a continuous activity source. The host owns the Markdown picker, preview, per-file inclusion, identity confirmation, progress, retry, deletion, and memory handoff. It accepts `.md` and `.markdown` files or folders, presents the task as importing journals, notes, or conversations rather than as a format-first workflow, detects chat-shaped records versus personal writing, and shows representative content before any memory write. The selected file subset is durable across navigation and refresh. Ordinary personal Markdown uses one file as one authored document: headings, date-like headings, and long sections remain structure inside that document and never create additional import records. Chat-shaped Markdown remains message-grained because speaker identity and conversation order are semantically meaningful. Long-document vector indexing may create internal retrieval chunks later, but every chunk remains attached to the same parent event and is collapsed back to that event for product-facing recall; chunks are not presented as imported content or separate memories. The preview therefore reports files, document preservation, and chat message counts rather than exposing internal record or chunk counts. Users must identify their own speaker names for conversations and confirm authorship for selected personal writing; files dominated by quotations, saved articles, or other people's writing should be excluded. Messages from other participants are retained only as conversation context and are not treated as claims about the user.

Confirmation prepares a recent raw slice for first-chat context without waiting for durable conclusions. It starts with up to 200 recent source records and may expand in bounded increments to at most 500 when needed to find roughly 30 meaningful user-authored chat messages or documents. The first-contact opening may use at most one low-sensitivity detail from this explicitly selected user writing, must treat archive text as data rather than instructions, and must not mention files, imports, memory, or sources. Thin, private, or ambiguous material is ignored rather than forcing personalization. The complete selection is then stored and submitted to the ordinary memory pipeline oldest-first within each source session. Chat dates come from explicit message timestamps or surrounding chat date headings when available. A document has only one document-level time: common frontmatter date fields, a dated filename, or file time as an explicitly approximate fallback. Body headings are content, not a document segmentation or timestamp contract. Missing chat timestamps preserve source order and remain explicitly approximate rather than becoming invented exact times.
