...
const FLOW = (fields: ExtensionFieldSpec[] = []): ActivationFlowSpec => ({
  ...
  authorize_on_confirm: false, enabled_key: 'sources.s.enabled', configured_key: 'sources.s.configured', fields,
});
const installed = (
  ...
  manifest: {
    ...
    author: '', official: false, contribution_types: ['source'], source: 'installed',
    ...
    settings_fields: [
      field('sources.s.enabled', { type: 'switch', default: false }),
      field('sources.s.configured', { type: 'switch', default: false }),
      ...settingsFields,
    ],
  },
...
const source = (overrides: Partial<SourceStatusItem> = {}): SourceStatusItem => ({
...
beforeEach(() => {
  ...
  vi.spyOn(sourcesApi, 'getStatus').mockImplementation(async () => ({
    sources: [
  ...
  vi.spyOn(sourcesApi, 'requestSync').mockImplementation(async (name, id) => {
    syncCounts.set(id, (syncCounts.get(id) ?? 0) + 1);
    return { source_name: name, connection_id: id, queued: true };
  });
  vi.spyOn(sourcesApi, 'requestAuthorization').mockResolvedValue({
    authorized: true, requested_types: [], granted_types: [], denied_types: [],
  });
  vi.spyOn(sourcesApi, 'getMemoryReadiness').mockImplementation(async (name, id) =>
...
describe('usePluginInstallFlow', () => {
  it('uses installed manifest metadata and scopes creation, sync and readiness to one new account', async () => {
    ...
    expect(pluginsApi.createConnection).toHaveBeenCalledExactlyOnceWith('p', {
      ...
      settings: { sources: { s: { enabled: true, configured: true } } }, credentials: {},
    });
    expect(sourcesApi.requestSync).toHaveBeenCalledWith('s', 'connection-p', undefined);
    expect(sourcesApi.getMemoryReadiness).toHaveBeenCalledWith('s', 'connection-p', { maxWaitMs: 1500 });
  ...
  it('collects canonical required fields before any connection, authorization or status lookup', async () => {
    ...
    expect(sourcesApi.requestAuthorization).not.toHaveBeenCalled();
    ...
    expect(pluginsApi.createConnection).toHaveBeenCalledWith('p', {
      display_name: 'Plugin p', enabled: true,
      settings: { directory: '/accounts/work', sources: { s: { enabled: true, configured: true } } },
      credentials: { 'account.token': 'private-token' },
    });
    expect(sourcesApi.requestAuthorization).toHaveBeenCalledWith('s', 'connection-p', expect.objectContaining({ directory: '/accounts/work' }));
    expect(result.current.phase).toBe('done');
  });

  it('first context applies manifest defaults and caps without asking for optional default fields', async () => {
    const fields = [
      field('sources.s.lookback_days', { type: 'number', default: 7 }),
      field('sources.s.max_items_per_sync', { type: 'number', default: 500 }),
    ];
    ...
    expect(pluginsApi.createConnection).toHaveBeenCalledWith('p', expect.objectContaining({
      settings: { sources: { s: { enabled: true, configured: true, lookback_days: 7, max_items_per_sync: 200 } } },
    }));
    expect(sourcesApi.requestSync).toHaveBeenCalledWith('s', 'connection-p', { firstContext: true });
    expect(sourcesApi.getMemoryReadiness).toHaveBeenCalledWith('s', 'connection-p', { maxWaitMs: 0 });
  });

  it('first context applies declared overrides and still asks for required directories', async () => {
    const fields = [
      field('sources.s.paths', { type: 'tags', required: true }),
      field('sources.s.lookback_days', { type: 'number', default: 90 }),
      field('sources.s.max_items_per_sync', { type: 'number', default: 500 }),
    ];
    mockPackages(installed({ ...FLOW(fields), first_context: {
      max_items_per_sync: 75, settings_overrides: { 'sources.s.lookback_days': 14 },
    } }));
    const { result } = renderHook(() => usePluginInstallFlow('p', false, 'first_context'));
    await advance();
    expect(result.current.flow?.fields.map((item) => item.key)).toEqual(['sources.s.paths']);
    act(() => result.current.submitFields({ 'sources.s.paths': ['/history'] }));
    await advance(1500);
    expect(pluginsApi.createConnection).toHaveBeenCalledWith('p', expect.objectContaining({
      settings: { sources: { s: { enabled: true, configured: true, paths: ['/history'], lookback_days: 14, max_items_per_sync: 75 } } },
    }));
    expect(result.current.phase).toBe('done');
  });
  ...
  it('first context completes when L1 input is known without waiting for L2', async () => {
    ...
    expect(sourcesApi.getMemoryReadiness).toHaveBeenCalledOnce();
  });
  ...
  it('finishes empty memory input without a background organizing note', async () => {
    ...
    vi.mocked(sourcesApi.getMemoryReadiness).mockResolvedValue(readiness({ l1_event_count: 0, l2_ready: false, l2_total_count: 0, l2_processed_count: 0 }));
    const { result } = renderHook(() => usePluginInstallFlow('p', false));
    await advance(1500);
  ...
  it('polls bounded memory progress and refreshes completion in the background', async () => {
    vi.mocked(sourcesApi.getMemoryReadiness).mockResolvedValue(readiness({ l1_event_count: 3, l2_ready: false, l2_total_count: 3, l2_processed_count: 1, l2_remaining_count: 2 }));
    const { result } = renderHook(() => usePluginInstallFlow('p', false));
    ...
    vi.mocked(sourcesApi.getMemoryReadiness).mockResolvedValue(readiness({ l1_event_count: 3, l2_total_count: 3, l2_processed_count: 3 }));
    await advance(3000);
  ...
  it('rejects readiness for a different connection without showing its counts', async () => {
    vi.mocked(sourcesApi.getMemoryReadiness).mockResolvedValue(readiness({ connection_id: 'other-account', l1_event_count: 999 }));
    const { result } = renderHook(() => usePluginInstallFlow('p', false));
    await advance(1500);
  ...
  it('rejects a mismatched sync receipt instead of polling another account', async () => {
    vi.mocked(sourcesApi.requestSync).mockResolvedValue({ source_name: 's', connection_id: 'other-account', queued: true });
    const { result } = renderHook(() => usePluginInstallFlow('p', false));
    await advance();
    expect(result.current.phase).toBe('error');
  ...
  it('does not create a connection for an absent package or use live source activation metadata', async () => {
    mockPackages();
    const { result } = renderHook(() => usePluginInstallFlow('p', false));
    await advance();
    expect(result.current.phase).toBe('error');
  ...
  it('history import creates and returns an explicit connection without invoking source APIs', async () => {
    // Import-only packages can declare no source settings at all.
  ...
  it('installs with the confirmed fingerprint and retries sync without another install, account, or credential write', async () => {
    ...
    vi.mocked(sourcesApi.requestSync).mockRejectedValueOnce(new Error('Temporary sync failure'));
    ...
    expect(sourcesApi.requestSync).toHaveBeenCalledTimes(2);
  });
  ...
  it('isolates a late old create result from the next account, including its retry target', async () => {
    ...
    vi.mocked(sourcesApi.requestSync).mockRejectedValueOnce(new Error('Retry q'));
    ...
    expect(vi.mocked(sourcesApi.requestSync).mock.calls.every((call) => call[1] === 'connection-q')).toBe(true);
  });
  ...
  it('does not update the previous account after a retry read finishes during a switch', async () => {
    ...
    mockPackages(installed(), installed(FLOW(), [], 'q'));
    vi.mocked(sourcesApi.requestSync).mockRejectedValueOnce(new Error('Retry p'));
  ...
  it('does not authorize or sync an account after its delayed source lookup becomes stale', async () => {
    const oldStatus = deferred<SourceStatusResponse>();
    mockPackages(installed({ ...FLOW(), authorize_on_confirm: true }), installed(FLOW(), [], 'q'));
    vi.mocked(sourcesApi.getStatus).mockReturnValueOnce(oldStatus.promise);
    const { result, rerender } = renderHook(({ id }) => usePluginInstallFlow(id, false), { initialProps: { id: 'p' } });
    await advance();
    rerender({ id: 'q' });
    await advance(1500);
    ...
    expect(sourcesApi.requestAuthorization).not.toHaveBeenCalled();
    expect(sourcesApi.requestSync).toHaveBeenCalledExactlyOnceWith('s', 'connection-q', undefined);
  });
  ...
  it('keeps stale readiness and field submissions out of the new account', async () => {
    ...
    vi.mocked(sourcesApi.getMemoryReadiness).mockReturnValueOnce(oldReadiness.promise);
    const { result, rerender } = renderHook(({ id }) => usePluginInstallFlow(id, false), { initialProps: { id: 'p' } });
    await advance(1500);
  ...
  it('isolates a late create when the same package is closed and reopened for a different account', async () => {
    ...
    expect(sourcesApi.requestSync).toHaveBeenCalledExactlyOnceWith('s', 'connection-p', undefined);
  });
