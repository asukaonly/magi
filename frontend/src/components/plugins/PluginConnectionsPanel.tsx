            ...
            <div className="space-y-5 px-6 pb-6">
              ...
              {editor.connection ? <>
                <PluginSettingsCustomBlocks connectionId={editor.connection.connection_id} blocks={blocks}
                  values={Object.fromEntries(fields.map((field) => [field.key, readConnectionSetting(editor.settings, field.key)]))}
                  onChange={(key, value) => setEditor({ ...editor, settings: writeConnectionSetting(editor.settings, key, value) })} />
                <PluginSettingsActions key={editor.connection.connection_id} pluginId={pluginId} connectionId={editor.connection.connection_id}
                  actions={actions} values={Object.fromEntries(fields.map((field) => [field.key, readConnectionSetting(editor.settings, field.key)]))}
