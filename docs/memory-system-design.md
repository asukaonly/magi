...
- Embedding observation fields track vector readiness and active embedding profile state for the parent event
- `causation_id`, `trace_id`, `span_id`, and `parent_span_id` preserve event-envelope and runtime-trace linkage without making runtime trace a memory-truth store
