"""Internal capability ports for bundled tools and host composition only.

These adapters never cross the plugin process boundary. External plugins use
the bounded value contracts in magi_plugin_sdk.capabilities instead.
"""
...
class MemoryQueryPort(Protocol):
    ...
    The host adapter wraps the memory layer; bundled tools call through
...
class DetachPort(Protocol):
    ...
    The host adapter wraps the DetachSignal ContextVar; bundled tools call
...
class InteractionPort(Protocol):
    ...
    timeout — and returns an :class:`AskOutcome`. Bundled tools call this
