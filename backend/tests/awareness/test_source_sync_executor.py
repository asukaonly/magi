...
async def _wait_for_job_status(
    ...
    *,
    ...
    while time.time() < deadline:
        job = await repository.get_source_sync_job(job_id)
        if job is not None and job["status"] == expected_status:
            return job
        await asyncio.sleep(0.02)
    raise AssertionError(f"Timed out waiting for source sync job {job_id} to reach {expected_status}")
...
class _StoppingExecutor:
    def __init__(self) -> None:
        self.state = SourceSyncExecutorState.STOPPING
        self.timeout = True
...
@pytest.mark.asyncio
async def test_source_sync_executor_claims_and_completes_queued_job(tmp_path):
    ...
    assert completed_job["result_message"] == "source_sync_completed"
    ...
    assert execution_message == "source_sync_completed"


@pytest.mark.asyncio
async def test_source_sync_executor_paused_start_cannot_claim_before_resume(tmp_path):
    ...
    queued_job = await repository.get_source_sync_job(job_id)
...
@pytest.mark.asyncio
async def test_source_sync_executor_runs_jobs_on_owner_loop(tmp_path):
    db_path = tmp_path / "scheduler.db"
    repository = ScheduleRepository(db_path)
    await repository.initialize()
    schedule = _build_source_schedule()
    ...
    async def run_job(job_record: dict[str, object]) -> ScheduledExecutionResult:
        ...
        return ScheduledExecutionResult(success=True, message="source_sync_completed")
...
@pytest.mark.asyncio
async def test_source_sync_stop_timeout_retains_worker_and_blocks_restart(tmp_path):
    repository = ScheduleRepository(tmp_path / "scheduler.db")
    await repository.initialize()
    schedule = _build_source_schedule()
    job_id = await _enqueue_job(repository, schedule)
    ...
    try:
        ...
        assert executor.state is SourceSyncExecutorState.STOPPING
        ...
        release_run.set()
        if executor.state is not SourceSyncExecutorState.STOPPED:
            await executor.stop()

    completed_job = await _wait_for_job_status(repository, job_id, "success")
    assert completed_job["result_message"] == "released"
    assert run_calls == 1
    assert executor.state is SourceSyncExecutorState.STOPPED
...
@pytest.mark.asyncio
async def test_source_sync_executor_can_restart_after_timed_out_worker_exits(tmp_path):
    repository = ScheduleRepository(tmp_path / "scheduler.db")
    await repository.initialize()
    schedule = _build_source_schedule()
    job_id = await _enqueue_job(repository, schedule)
    ...
    await executor.start()
    assert executor.state is SourceSyncExecutorState.RUNNING
    await executor.stop()
    assert executor.state is SourceSyncExecutorState.STOPPED
    await executor.stop()
    assert executor.state is SourceSyncExecutorState.STOPPED
...
async def test_executor_module_keeps_timed_out_worker_for_later_shutdown() -> None:
    ...
    module = SourceSyncExecutorModule(context)
    ...
    assert context.agent_runtime.source_sync_executor is executor
...
@pytest.mark.asyncio
async def test_source_sync_success_triggers_l3_backfill(tmp_path, monkeypatch):
...
@pytest.mark.asyncio
async def test_source_sync_l3_backfill_does_not_block_next_source_job(tmp_path, monkeypatch):
    db_path = tmp_path / "scheduler.db"
    repository = ScheduleRepository(db_path)
    await repository.initialize()
    first_schedule = _build_source_schedule(source_type="slow-backfill-source")
    second_schedule = _build_source_schedule(source_type="next-source")
...
@pytest.mark.asyncio
async def test_source_sync_success_triggers_l2_derive(tmp_path, monkeypatch):
...
@pytest.mark.asyncio
async def test_source_sync_has_more_queues_continuation_and_defers_derivations(tmp_path, monkeypatch):
    db_path = tmp_path / "scheduler.db"
    repository = ScheduleRepository(db_path)
    await repository.initialize()
    schedule = _build_source_schedule()
...
@pytest.mark.asyncio
async def test_source_sync_continuation_preserves_backfill_request(tmp_path, monkeypatch):
    db_path = tmp_path / "scheduler.db"
    repository = ScheduleRepository(db_path)
    await repository.initialize()
    sync_request = {
    ...
    schedule = _build_source_schedule(target_payload={"sync_request": sync_request})
    job_id = await _enqueue_job(repository, schedule)
...
@pytest.mark.asyncio
async def test_first_context_source_sync_does_not_queue_continuation(tmp_path, monkeypatch):
    db_path = tmp_path / "scheduler.db"
    repository = ScheduleRepository(db_path)
    await repository.initialize()
    schedule = _build_source_schedule(target_payload={"first_context": True})
    job_id = await _enqueue_job(repository, schedule)
...
@pytest.mark.asyncio
async def test_source_sync_success_no_scheduler_service(tmp_path, monkeypatch):
...
@pytest.mark.asyncio
async def test_source_sync_l2_derive_trigger_failure_does_not_fail_sync(tmp_path, monkeypatch):
...
@pytest.mark.asyncio
async def test_source_sync_executor_recovers_recent_running_job_on_startup(tmp_path):
    db_path = tmp_path / "scheduler.db"
    repository = ScheduleRepository(db_path)
    await repository.initialize()
    schedule = _build_source_schedule()
    job_id = await _enqueue_job(repository, schedule)
    claimed = await repository.claim_next_source_sync_job(claimed_by="stale-executor")
...
@pytest.mark.asyncio
async def test_source_sync_executor_retries_transient_failure_until_success(tmp_path):
    db_path = tmp_path / "scheduler.db"
    repository = ScheduleRepository(db_path)
    await repository.initialize()
    schedule = _build_source_schedule()
...
@pytest.mark.asyncio
async def test_source_sync_executor_fails_after_retry_budget_is_exhausted(tmp_path):
    db_path = tmp_path / "scheduler.db"
    repository = ScheduleRepository(db_path)
    await repository.initialize()
    schedule = _build_source_schedule()
...
@pytest.mark.asyncio
async def test_source_sync_executor_retries_when_success_settlement_is_temporarily_unavailable(
    tmp_path,
    monkeypatch,
):
    repository = ScheduleRepository(tmp_path / "scheduler.db")
    ...
    settle_success = repository.settle_source_sync_job_success
    ...
    monkeypatch.setattr(
        repository,
        "settle_source_sync_job_success",
...
@pytest.mark.asyncio
async def test_source_sync_executor_recovers_when_failure_settlement_is_interrupted(
    tmp_path,
    monkeypatch,
):
    repository = ScheduleRepository(tmp_path / "scheduler.db")
    ...
    run_attempts = 0
    settlement_attempts = 0
    settle_failure = repository.settle_source_sync_job_failure
    ...
    monkeypatch.setattr(
        repository,
        "settle_source_sync_job_failure",
