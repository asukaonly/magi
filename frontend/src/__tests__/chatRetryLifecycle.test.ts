import { centerLocalStorage, centerSessionStorage, centerStorageKey, setCenterStorageScope } from '@/runtime/center-storage';
import {
  afterAll,
  afterEach,
  beforeEach,
...
vi.mock('@/api/modules/server', () => ({ serverApi: { clear: async (id: string) => ({ version: 2, kind: 'clear', operation_id: id, phase: 'completed', data_epoch: id, content_epoch: id, result: await clearAllMock(), error: null }), info: async () => ({ service_ready: true, maintenance: { data_epoch: 'cleared', content_epoch: 'cleared' } }) } }));
...
describe('chat retry lifecycle', () => {
  ...
  it('clears persisted retries and cached sessions before announcing a successful full clear', async () => {
    ...
    window.localStorage.setItem(
      centerStorageKey('magi.first-context-continuation:session-a'),
      JSON.stringify({ version: 1, mode: 'dismissed' }),
    );
    window.localStorage.setItem(
      centerStorageKey('magi.composer.mru.mentions'),
      JSON.stringify(['filesystem|file:///private/report.txt']),
    );
    window.localStorage.setItem(
      centerStorageKey('magi.chat.readCursors.v1'),
      JSON.stringify({ 'session-a': { messageCount: 1, lastTimestamp: 1 } }),
    );
    window.localStorage.setItem(centerStorageKey('magi.chat.readCursors.initialized.v1'), 'true');
    window.localStorage.setItem(
      centerStorageKey('magi.desktopNotifications.sent.v1'),
    ...
    expect(centerLocalStorage().getItem('magi_onboarding_state')).toBeNull();
    expect(window.localStorage.getItem(
      centerStorageKey('magi.first-context-continuation:session-a'),
    )).toBeNull();
    expect(window.localStorage.getItem(centerStorageKey('magi.composer.mru.mentions'))).toBeNull();
    expect(window.localStorage.getItem(centerStorageKey('magi.chat.readCursors.v1'))).toBeNull();
    expect(window.localStorage.getItem(centerStorageKey('magi.chat.readCursors.initialized.v1'))).toBeNull();
    expect(window.localStorage.getItem(centerStorageKey('magi.desktopNotifications.sent.v1'))).toBeNull();
