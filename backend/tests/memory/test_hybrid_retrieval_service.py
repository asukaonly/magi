...
class TestL2BacktrackQueries:
    """Tests for _extract_l2_backtrack_queries static method."""

    def test_extracts_from_relationships(self):
        payload = RetrievalPayload()
        payload.l2_relationships = [
            {"natural_summary": "User has three sisters", "evidence_text": "I have 3 sisters"},
            {"natural_summary": "User plays guitar", "evidence_text": "I play guitar every day"},
        ]
        queries = HybridRetrievalService._extract_l2_backtrack_queries(payload)
        assert "User has three sisters" in queries
        assert "I have 3 sisters" in queries
        assert "User plays guitar" in queries
        assert "I play guitar every day" in queries

    def test_extracts_from_assertions(self):
        payload = RetrievalPayload()
        payload.l2_assertions = [
            {"trait_name": "sibling_count", "trait_value": "3"},
            {"trait_name": "hobby", "trait_value": "playing guitar"},
        ]
        queries = HybridRetrievalService._extract_l2_backtrack_queries(payload)
        assert "sibling_count 3" in queries
        assert "hobby playing guitar" in queries

    def test_deduplicates_case_insensitive(self):
        payload = RetrievalPayload()
        payload.l2_relationships = [
            {"natural_summary": "User plays guitar", "evidence_text": "user plays guitar"},
        ]
        queries = HybridRetrievalService._extract_l2_backtrack_queries(payload)
        assert len(queries) == 1

    def test_skips_empty_and_short(self):
        payload = RetrievalPayload()
        payload.l2_relationships = [
            {"natural_summary": "", "evidence_text": None},
            {"natural_summary": "x", "evidence_text": "ab"},
        ]
        payload.l2_assertions = [
            {"trait_name": "", "trait_value": ""},
        ]
        queries = HybridRetrievalService._extract_l2_backtrack_queries(payload)
        assert "ab" in queries
        assert len(queries) == 1
