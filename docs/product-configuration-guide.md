...
The history-import path is a one-shot transfer rather than a continuous activity source. The host owns the Markdown picker, preview, per-file inclusion, identity confirmation, progress, retry, deletion, and memory handoff. It accepts `.md` and `.markdown` files or folders, presents the task as importing journals, notes, or conversations rather than as a format-first workflow, detects chat-shaped records versus personal writing, and shows representative content before any memory write. The selected file subset is durable across navigation and refresh. Users must identify their own speaker names for conversations and confirm authorship for selected personal writing; files dominated by quotations, saved articles, or other people's writing should be excluded. Messages from other participants are retained only as conversation context and are not treated as claims about the user.

Confirmation prepares a recent raw slice for first-chat context without waiting for durable conclusions. It starts with up to 200 recent records and may expand in bounded increments to at most 500 when needed to find roughly 30 meaningful user-authored records. The first-contact opening may use at most one low-sensitivity detail from this explicitly selected user writing, must treat archive text as data rather than instructions, and must not mention files, imports, memory, or sources. Thin, private, or ambiguous material is ignored rather than forcing personalization. The complete selection is then stored and submitted to the ordinary memory pipeline oldest-first within each source session. Source dates are taken from explicit message timestamps, common frontmatter date fields, dated filenames, or dated headings when available. Missing timestamps preserve source order and remain explicitly approximate rather than becoming invented exact times.
