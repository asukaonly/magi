...
from magi.memory.l2.assertions.conflict_notification_display import (
    ...
    render_profile_conflict_notification,
)
...
async def _conflict(store, *, summary: bool = True):
    ...
    base = {
        ...
        "target_entity_id": "food:opaque", "target_entity_type": "food",
        "target_scope": "entity_bound", "temporal_scope": "recent",
        "confidence_score": 0.4, "volatility_index": 0.2,
        "inference_depth": "explicit", "validation_state": "tentative",
    ...
    authoritative = await store.upsert_assertion_candidate({
        **base, "trait_value": "dislike", "source_domain": "user_authored",
    ...
    shadow = await store.upsert_assertion_candidate({
        **base, "trait_value": "like", "source_domain": "external_activity",
        "evidence_events": ["shadow-evidence"],
        "natural_summary": "用户最近喜欢草莓。" if summary else "",
        "first_inferred_at": 1710000100.0, "last_validated_at": 1710000100.0,
    })
    assert (await store.get_tom_assertion(assertion_id=shadow))["status"] == "shadow"
    return authoritative, shadow


def _notification_service(tmp_path, *, authoritative: str = "missing-auth", shadow: str = "missing-shadow"):
    store = NotificationStore(str(tmp_path / "notifications.db"))
    store.ensure_schema()
    payload = {
        "conflict_type": "profile_conflict", "authoritative_id": authoritative,
        "shadow_id": shadow, "authoritative_value": "dislike", "inferred_value": "like",
        "trait_name": "preference.affinity", "entity_id": "user:local_user",
    }
    notification_id = store.insert(NotificationRow(
        user_id="default_user", kind="suggestion",
        ...
        payload_json=json.dumps(payload), created_at_ms=1000,
    ))
...
async def _public_feed(service, memory, *, suppressed: bool = False):
    ...
    router = build_default_notifications_router(
        service_dep=lambda: service, unified_memory_dep=lambda: memory,
...
async def test_existing_raw_notification_projects_facts_without_rewriting_data(l2_store_with_schema, tmp_path, summary):
    memory = l2_store_with_schema
    authoritative, shadow = await _conflict(memory, summary=summary)
    store, service, notification_id, payload = _notification_service(tmp_path, authoritative=authoritative, shadow=shadow)
...
@pytest.mark.asyncio
async def test_new_notifications_use_the_same_complete_fact_renderer(l2_store_with_schema, tmp_path):
    memory = l2_store_with_schema
    await _conflict(memory)
    store = NotificationStore(str(tmp_path / "notifications.db"))
    store.ensure_schema()
    stats = await materialize_shadow_conflict_notifications(
        memory, NotificationService(store=store), user_id="default_user", entity_id="user:local_user",
    )
...
async def test_invisible_assertion_never_reappears_from_stored_notification(l2_store_with_schema, tmp_path, side, mutation):
    memory = l2_store_with_schema
    ...
    async with sqlite_connection_async(memory.db_path) as db:
        await db.execute(f"UPDATE tom_trait_assertions SET {mutation} WHERE assertion_id = ?", (assertion_id,))
        await db.commit()
...
async def test_missing_memory_and_missing_ids_have_honest_fallback(tmp_path):
    ...
    assert page["items"][0]["body"] == "已有记录：这条记录缺少完整事实描述。\n待确认候选：这条记录缺少完整事实描述。"
...
async def test_notification_renderer_preserves_unresolved_subject_and_locale():
    row = {
        ...
        "trait_name": "preference.affinity", "trait_value": "dislike",
        "target_entity_id": "food:private", "temporal_scope": "recent", "status": "shadow",
    }
    title, body = await render_profile_conflict_notification(
        db_path=None, shadow=row, authoritative=None, language="en",
    )
...
@pytest.mark.asyncio
async def test_conflicts_with_diverged_slots_do_not_compare_unrelated_facts(l2_store_with_schema, tmp_path):
    memory = l2_store_with_schema
    authoritative, shadow = await _conflict(memory)
    _, service, _, _ = _notification_service(tmp_path, authoritative=authoritative, shadow=shadow)
    async with sqlite_connection_async(memory.db_path) as db:
        await db.execute("UPDATE tom_trait_assertions SET target_entity_id = 'food:other' WHERE assertion_id = ?", (shadow,))
        await db.commit()
    item = (await _public_feed(service, SimpleNamespace(l2=memory)))["items"][0]
...
@pytest.mark.asyncio
async def test_page_batches_unique_assertion_ids_and_catalog_reads(l2_store_with_schema, monkeypatch):
    ...
    connect = Mock(wraps=display.sqlite_connection_async)
    decorate = Mock(wraps=display.decorate_assertion_display)
    monkeypatch.setattr(display, "sqlite_connection_async", connect)
    monkeypatch.setattr(display, "decorate_assertion_display", decorate)
    payloads = [{"authoritative_id": authoritative, "shadow_id": f"missing-{i}"} for i in range(401)]
...
async def test_restore_list_projects_existing_conflict_titles_without_config_writes(monkeypatch, old_title):
    ...
    record = DismissalRecord(
        dedupe_key="profile_conflict:preference.affinity:food:opaque",
        dismissed_at=datetime.now(timezone.utc), kind="explicit", title=old_title,
    )
    monkeypatch.setattr(dismissals, "list_active_dismissals", lambda: [record])
    app = FastAPI()
    app.include_router(_build_public_router(system_suggestions_router, _PUBLIC_ROUTE_METHODS["system_suggestions"]))
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
